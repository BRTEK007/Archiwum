...
window.addEventListener('keydown', function (event) {
  ...
  switch (event.key) {
    ...
    case "r": event.preventDefault(); if (keys[R_KEY].isPressed) break; keys[R_KEY].pressed = true; keys[R_KEY].isPressed = true; break;
  }
...
const ScreenDrawer = function () {
  ...
  this.clear = function (color = "black") {
  ...
  this.invokeShake = function (strength, duration) {
  ...
  this.drawTiles = function () {
    ...
    for (var x = xRange.start; x <= xRange.end; x++) {
      for (var y = yRange.start; y <= yRange.end; y++) {
        var tileId = y * level.width + x;
        var tileValue = level.tiles[tileId];
        if (tileValue != false) {
...
const Game = function (l) {
  ...
  this.objectColliders = new Array();
  ...
  this.update = function () {
    ...
    this.player.update();
    this.screenDrawer.update(this.player);
    ...
    for (let i = 0; i < this.enemyBullets.length; i++) {
      ...
      var tiles = getWallsInRect(bullet.getMomentumRect());
      for (var j = 0; j < tiles.length; j++) {

        var framesToCollision = dynamicRectTileCollisionTime(bullet, tiles[j]);
    ...
    for (let i = 0; i < this.objectColliders.length; i++) {
      this.objectColliders[i].draw(this.screenDrawer);
    }
    ...
    //object colliders
    for (let i = 0; i < this.objectColliders.length; i++) {
      limitPlayerRectVelocity(this.player, this.objectColliders[i]);
    }
    ...
    for (let i = 0; i < this.enemies.length; i++) {
      ...
      var framesToCollision = dynamicRectsCollisionTime(this.player.getCollider(), this.enemies[i].getCollider());
    ...
    for (let i = 0; i < this.enemyBullets.length; i++) {
      ...
      var framesToCollision = dynamicRectsCollisionTime(this.player.getCollider(), bullet.getCollider());
      if (framesToCollision <= 1) {
        this.restartLevel();
        this.enemyBullets.splice(i, 1);
        i--;
    ...
    var objects = getObjectsInRect(this.player.getMomentumRect());

    for (let i = 0; i < objects.length; i++) {
      var object = objects[i];
      var framesToCollision = dynamicRectsCollisionTime(this.player.getCollider(), object.collider);
    ...
    for (let i = 0; i < this.coins.length; i++) {
      if(!this.coins[i].active) continue;
      var framesToCollision = dynamicRectsCollisionTime(this.player.getCollider(), this.coins[i].getCollider());
...
Game.Player = function (x, y) {
  ...
  this.draw = function (viewManager) {
    if (this.dead) return;
...
const PartFactory = {
  ...
  rocketLaunchPart: (x, y, direction) => ({
    launch: () => {
      var bullet = EntityFactory.RocketBullet(x, y, direction);
  ...
  dropperLaunchPart: self => ({
    launch: () => {
      var bullet = EntityFactory.DropperBullet(self.pos.x + 2, self.pos.y + 8);
      bullet.setVel(0, 2);//2
  ...
  drawNoAnimPart: (self, sourceX, sourceY, sourceW, sourceH) => ({
    draw: (camera) => {
      let screenX = self.pos.x - camera.cameraPos.x + camera.size.x / 2;
      let screenY = self.pos.y - camera.cameraPos.y + camera.size.y / 2;
  ...
  dropperUpdatePart: (self, launchDelay) => ({
    update: () => {
      self.launchTimeCounter++;
      let delay = self.animFrame == 0 ? launchDelay : 3;
...
function dynamicRectTileCollisionTime(rect, tile) {
  ...
  var nearestCollisionFrames = Infinity;

  var xColFrames = Infinity;
    ...
    for (var y = yRange.start; y <= yRange.end; y++) {
      ...
      if (level.tiles[tileId] != false && level.tiles[tileId].type != TILE_WALL) {
        ...
        if (level.tiles[tileId].type == TILE_SPIKES) {
          var sides = level.tiles[tileId].sides;
...
function limitPlayerTileVelocity(player) {
  ...
  var tiles = getWallsInRect(player.getMomentumRect());
...
function limitPlayerRectVelocity(player, rect) {
  if (isInRange(player.pos.x + player.vel.x, rect.pos.x - player.size.x, rect.pos.x + rect.size.x)) {//vertical
    if (player.vel.y > 0 && rect.pos.y >= player.pos.y + player.size.y) {//bottom
      player.vel.y = Math.min(player.vel.y, rect.pos.y - (player.pos.y + player.size.y));
      if (player.vel.y == 0) player.grounded = true;
    }
    else if (player.vel.y < 0 && rect.pos.y + rect.size.y <= player.pos.y)//top
      player.vel.y = Math.max(player.vel.y, rect.pos.y + rect.size.y - player.pos.y);
  }

  if (isInRange(player.pos.y + player.vel.y, rect.pos.y - player.size.y, rect.pos.y + rect.size.y)) {//vertical
    if (player.vel.x > 0 && rect.pos.x >= player.pos.x + player.size.x)//right
      player.vel.x = Math.min(player.vel.x, rect.pos.x - (player.pos.x + player.size.x));
    else if (player.vel.x < 0 && rect.pos.x + rect.size.x <= player.pos.x)//left
      player.vel.x = Math.max(player.vel.x, rect.pos.x + rect.size.x - player.pos.x);
  }
}