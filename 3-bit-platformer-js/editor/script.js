...
function buttonReload() {
  for (var i = 0; i < level.tiles.length; i++) {
    ...
    let tileX = i % level.width;
    let tileY = Math.floor(i / level.width);

    if (level.tiles[i].type == TILE_WALL)
      level.tiles[i] = getWallTileValue(tileX, tileY);
    else if (level.tiles[i].type == TILE_SPIKES)
      level.tiles[i] = getSpikesTileValue(tileX, tileY);
  }
}
...
function buttonResizeConfirm(){
  var right = parseInt(document.getElementById('resizeRightInput').value);
  var left = parseInt(document.getElementById('resizeLeftInput').value);
  var top = parseInt(document.getElementById('resizeTopInput').value);
  var bottom = parseInt(document.getElementById('resizeBottomInput').value);

  var oldTiles = level.tiles.slice(0);

  var oldWidth = level.width;
  var oldHeight = level.height;

  level.width += right + left;
  level.height += top + bottom;
  level.tiles = new Array(level.width * level.height).fill(false);

  for (var y = top; y < oldHeight + top; y++) {
    for (var x = left; x < oldWidth + left; x++) {
      var setId = y * level.width + x;
      var getId = (y - top) * oldWidth + (x - left);

      level.tiles[setId] = oldTiles[getId];
    }
  }

  level.startX += left * 16;
  level.startY += top * 16;

  level.endX += left * 16;
  level.endY += top * 16;

  closePopup('resizePopup');
}

function buttonResize() {
  var el = document.getElementById('resizePopup');
  el.style.display = 'inline-block';
  let rect = canvas.getBoundingClientRect();
...
function buttonTrim() {
  ...
  var xRange = { min: oldWidth - 1, max: 0 };
  var yRange = { min: oldHeight - 1, max: 0 };

  for (var y = 0; y < oldHeight; y++) {
    for (var x = 0; x < oldWidth; x++) {

      var tileId = y * oldWidth + x;

      if (oldTiles[tileId] != false) {
  ...
  xRange.min = Math.floor(Math.min(xRange.min, level.startX / 16, level.endX / 16));
  xRange.max = Math.floor(Math.max(xRange.max, level.startX / 16, level.endX / 16));

  yRange.min = Math.floor(Math.min(yRange.min, level.startY / 16, level.endY / 16));
  yRange.max = Math.floor(Math.max(yRange.max, level.startY / 16, level.endY / 16));


  if (xRange.min >= xRange.max || yRange.min >= yRange.max) return;

  var newWidth = xRange.max - xRange.min + 1;
  var newHeight = yRange.max - yRange.min + 1;

  level.width = newWidth;
  level.height = newHeight;

  level.tiles = new Array(newWidth * newHeight).fill(false);

  for (var y = 0; y < level.height; y++) {
    for (var x = 0; x < level.width; x++) {
      var setId = y * level.width + x;
      var getId = (y + yRange.min) * oldWidth + (x + xRange.min);

      level.tiles[setId] = oldTiles[getId];
    }
  }

  level.startX -= xRange.min * 16;
  level.startY -= yRange.min * 16;

  level.endX -= xRange.min * 16;
  level.endY -= yRange.min * 16;
}

function buttonBorders(){

  for(var x = 0; x < level.width; x++){
    var tileId = 0 * level.width + x;
    if(level.tiles[tileId] == false){
      level.tiles[tileId] = getWallTileValue(x, 0);
      updateTilesAround(tileId);
    }
    tileId = (level.height-1) * level.width + x;
    if(level.tiles[tileId] == false){
      level.tiles[tileId] = getWallTileValue(x, level.height-1);
      updateTilesAround(tileId);
    }
  }
  for(var y = 1; y < level.height-1; y++){
    var tileId = y * level.width + 0;
    if(level.tiles[tileId] == false){
      level.tiles[tileId] = getWallTileValue(0, y);
      updateTilesAround(tileId);
    }
    tileId = y * level.width + level.width-1;
    if(level.tiles[tileId] == false){
      level.tiles[tileId] = getWallTileValue(level.width-1, y);
      updateTilesAround(tileId);
    }
  }
}
